...
(function () {
  ...
  function iconKey(name) {
    ...
    if (/кардио|кардіо/.test(n)) return "cardio";
    if (/молитва|в висе|у висі|скручиван|скручуван|прес/.test(n)) return "abs";
    // ноги проверяем РАНЬШЕ бицепса/трицепса (иначе «на бицепс бедра» ловит curl)
    if (/присед|присід|выпад|випад|жим ногами|разгибания ног|розгинання ніг|сгибания ног|згинання ніг|носки|носк|мертвая|мертва|румынск|румунськ|гакк|ягодич|сідничн|місток|екстензія|отведение ног|відведення ноги|разведение ног|розведення ніг|сведение ног|зведення ніг/.test(n)) return "legs";
    if (/бицепс|біцепс|молотк|сгибания рук|згинання рук/.test(n)) return "curl";
    if (/трицепс|французск|французьк|разгибания в блоке|розгинання в блоц|разгибания из-за|розгинання із-за|разгибания с двух|розгинання з двох|узким хватом|вузьким хватом|отжимания с упором|віджимання з упором/.test(n)) return "triceps";
    if (/подтягив|підтягуван|вертикальн|пуловер|пулловер|тяга верхн/.test(n)) return "pulldown";
    if (/мах|отведение|відведення|протяжк|задн|разведение|розведення|подъемы рук|підйоми рук/.test(n)) return "delts";
    if (/тяга|горизонтальн/.test(n)) return "row";
    if (/жим.*сид|жим.*сидяч/.test(n)) return "shoulder";
    return "chest";
  }
